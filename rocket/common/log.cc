...
Logger::Logger(LogLevel level, int type) : m_set_level(level), m_type(type) {
    ...
    m_asnyc_app_logger = std::make_shared<AsyncLogger>(Config::GetGobalConfig()->m_log_file_name + "_app",
                                                   Config::GetGobalConfig()->m_log_file_path,
                                                   Config::GetGobalConfig()->m_log_max_file_size);
...
void *AsyncLogger::Loop(void *arg) {
    ...
    while (1) {
        ...
        std::stringstream ss;
        ss << logger->m_file_path << logger->m_file_name << std::string(date) << "_log.";
