...
echo "Run rocket project, name: ${PROJECT_NAME}, path: ${PROJECT_BIN_FILE}"
...
then
  echo "Run rocket eror, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
...
then
  echo "Run rocket eror, file: ${PROJECT_CONF_FILE} not exist, please check config file"
  exit -1
fi
...
echo "Start rocket server ${PROJECT_CONF_FILE} succ"
